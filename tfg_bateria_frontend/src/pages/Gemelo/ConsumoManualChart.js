...
export default function ChartsConsumo({ consumoManual = [] }) {
  const [series, setSeries] = useState([]);
  const [range,  setRange]  = useState([null, null]);
  const [loading, setLoading] = useState(true);

  // Cuando cambian los datos de consumoManual, reprocesamos y centramos el rango
  useEffect(() => {
    setLoading(true);

    if (consumoManual.length === 0) {
      // no hay nada que procesar
      setLoading(false);
      return;
    }
    if (consumoManual.length === 0) return;
    const pts = process(consumoManual);
    setSeries(pts);
