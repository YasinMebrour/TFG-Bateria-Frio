...
from app.config import XGB_MODEL_PATH, BUCKET_INFLUX_CAMARA, NUM_LAGS
...
RUTA_MODELO_XGB = str(XGB_MODEL_PATH)
TIEMPO_ACTUAL = None  # actualizado en prediccion_recursiva_xgb
...
def cargar_y_preprocesar_datos(start_dt: pendulum.DateTime | None):
    ...
    if start_dt is None:
        ...
        fetch_start = _to_rfc3339_utc(start_dt.subtract(minutes=NUM_LAGS + 180))
...
def prediccion_recursiva_xgb(model, ultimas_caracteristicas, pasos, modos_futuros):
    """
    Devuelve ``np.array(pasos, 3)`` con las predicciones de
    [consumo, humedad, temperatura] minuto a minuto.
    """
    global TIEMPO_ACTUAL
    predicciones = []

    idx_c0, idx_c1 = 11, 11 + NUM_LAGS
    idx_h0, idx_h1 = idx_c1, idx_c1 + NUM_LAGS
    idx_t0, idx_t1 = idx_h1, idx_h1 + NUM_LAGS
    ...
    for i in range(pasos):
        ...
        TIEMPO_ACTUAL = TIEMPO_ACTUAL.add(minutes=1)
        h, m = TIEMPO_ACTUAL.hour, TIEMPO_ACTUAL.minute
...
def generar_predicciones(schedule: list, start_date: str | None = None):
    ...
    # ---------- modelo ----------
    if not os.path.exists(RUTA_MODELO_XGB):
        raise FileNotFoundError(f"No se encontró el modelo: {RUTA_MODELO_XGB}")
    with open(RUTA_MODELO_XGB, "rb") as f:
        model = pickle.load(f)
    ...
    # ---------- instante inicial ----------
    # Queremos que TIEMPO_ACTUAL sea el último minuto del día anterior
    ...
    row = df_hist.iloc[-1]
    TIEMPO_ACTUAL = pendulum.instance(row.name).in_timezone("Europe/Madrid")
    inicio_prediccion = TIEMPO_ACTUAL
    ...
    lags = np.hstack((
        np.full(NUM_LAGS, row["consumo"]),
        np.full(NUM_LAGS, row["humedad"]),
        np.full(NUM_LAGS, row["temperatura"]),
    ))
    ...
    modos_futuros = []
    for i in range(1440):
        t = TIEMPO_ACTUAL.add(minutes=i+1)
